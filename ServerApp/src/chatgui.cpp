...
bool ChatBotApp::OnInit()
{
    ...
    ChatBotFrame *chatBotFrame = new ChatBotFrame(wxT("ChatAppServer"));
...
void ChatBotPanelDialog::ReceivingThread(){
    ...
    while(true){
        if(_serverSocket->recv(msg, size) <= 0){
            _timer.Stop();
            break;
        }
        else if (strncmp(msg, "CLOSEAPP", 9) == 0) {
            _serverSocket->send("CLOSEAPP", 9);
            break;
        }
...
void ChatBotPanelDialog::Send(wxString msg){
    ...
    if (_serverSocket->send(str_msg.c_str(), str_msg.length()) != SOCKET_ERROR) {
        AddDialogItem(msg, true);
    }
    else {
        wxMessageBox(wxT("Error in sending the message"), wxT("ChatAppServer Error"), wxICON_ERROR);
    }
}
...
void ChatBotPanelDialog::Close(){
    _timer.Stop();
    _serverSocket->send("CLOSEAPP", 9);     // message to wake up the waiting recv call in the thread
    _serverSocket->closeserver();
    _serverSocket->close(); //closing the socket
    ...
    : wxScrolledWindow(parent, id), _timer(this, TIMER_ID)
{
    ...
    _serverSocket = std::make_unique<SimpleTCPServerSocket>(); 

    // pass pointer to chatbot dialog so answers can be displayed in GUI
    if(!_serverSocket->bind()){
        wxMessageBox( wxT("Error in binding socket"), wxT("ChatAppServer error"), wxICON_ERROR);
        return;
    }
    if(!_serverSocket->listen()){
        wxMessageBox( wxT("Error in listening to socket"), wxT("ChatAppServer error"), wxICON_ERROR);
        return;        
    }
